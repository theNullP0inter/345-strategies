...
- **Chart types without a scheduled close**: on Renko, Kagi, Line Break, Point & Figure and Range charts the forming bar's `time_close` is `na`, so Auto never arms and `shouldStraddle` never fires there (under `On`, only daily and intraday slots still preview). Those chart types are unsupported: `syminfo.tickerid` hands the chart-type-modified series to the HTF slots. Heikin Ashi is time-based and unaffected.
